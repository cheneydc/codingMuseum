...
- **JS** — 语言切换 + 导航高亮 + 滚动渐入动画 + 手机端卡片追踪高亮，在 `script.js`
...
└── assets/
    └── images/             # 所有图片资源（15 个文件）
        ├── eniac.jpg              # 1940s - ENIAC 真实照片
        ├── assembly_code.jpg      # 1940s - Machine Code 汇编代码
        ├── ibm_card_punch.jpg     # 1940s - IBM 029 打孔机
        ├── mainframe.jpg          # 1950s - FORTRAN IBM 704 大型主机
        ├── cobol_code.jpg         # 1950s - COBOL 程序代码
        ├── lisp_machine.jpg       # 1950s - Lisp Machine（左图）
        ├── lisp_machine2.jpg      # 1950s - Lisp Machine（右图）
        ├── algol_punchcard.jpg    # 1960s - ALGOL 打孔卡片
        ├── dennis_ritchie.jpg     # 1970s - Dennis Ritchie（C 语言之父）
        ├── bjarne_stroustrup.jpg  # 1980s - Bjarne Stroustrup（C++ 之父）
        ├── python_logo.png        # 1990s - Python Logo（右上角）
        ├── java_logo.png          # 1990s - Java Logo（右上角）
        ├── js_shield.svg          # 1990s - JavaScript 盾牌 Logo（右上角）
        ├── datacenter.jpg         # 2000s - 数据中心（横幅图片）
        ├── matrix_rain.jpg        # 2020s - 代码雨背景
        ├── favicon.svg            # 浏览器标签栏图标
        ├── wechat_reward.png      # 微信打赏码
        └── alipay_reward.png      # 支付宝打赏码
```
...
手机端替代桌面端 decade-nav 的下拉菜单（仅在 ≤767px 显示）：

```html
<!-- 放在 header 的 mobile-nav-wrap 内 -->
<button class="mobile-nav-trigger">
    <span class="mobile-nav-dot"></span>
    <span class="mobile-nav-label">1960s</span>
</button>
<div class="mobile-timeline">
    <a href="#1960s" class="mobile-timeline-item active">1960s</a>
    ...
</div>
```

特点：
- 圆点 + 当前年代文字，点击展开列表，选择后自动收起
- 下拉列表使用 `position: fixed; top: 46px; right: 0.75rem`，不改变页面布局
- 无外边框、无三角箭头
- 列表项有圆点，活跃项发光
- 点击页面其他位置自动收起
...
台式端保持不变（在 header 内），手机端替换为右下角浮动圆按钮：

```html
<button class="mobile-lang-toggle">EN</button>  <!-- EN / 中 -->
```

CSS 样式：
- 桌面端（>767px）隐藏，手机端（≤767px）固定右下角
- 圆角 34×34、半透明毛玻璃效果、金色文字
- `backdrop-filter: blur(4px)` + 按压缩放反馈
...
- 默认内容为英文版
- 中文使用中文双引号 `""`，英文用单引号 `''`
- 幽默调侃风格，代号 0531 的博物馆 AI 管理员声音
- 包含历史准确信息和趣味吐槽

### AI 管理员声音

全部卡片文案使用统一的 AI 博物馆管理员口吻，入口声明在首页标题下方：

```html
<p class="hero-curator" data-zh="博物馆0531号AI管理员 发掘整理"
   data-en="Excavated & curated by AI Curator No. 0531"></p>
```
...
桌面端保持不变（hover 才高亮）。手机端（≤767px）通过 JavaScript 实现自动卡片高亮：

- **算法**：`scroll` / `resize` / `touchmove` 事件触发，`requestAnimationFrame` 节流
- **逻辑**：计算所有卡片的 `getBoundingClientRect()`，找到距离屏幕中线最近的一张
- **样式**：添加 `.card-visible` 类，CSS 自动渲染为金色边框+上浮+光影+图片高亮
- **无设备判断**：JS 在全部设备上运行，但 `.card-visible` 只在手机媒体查询中生效

关键 CSS 规则：
```css
@media (max-width: 767px) {
    html .language-card.card-visible {
        transform: translateY(-4px) !important;
        border-color: var(--accent-gold) !important;
        box-shadow: 0 10px 35px rgba(0, 0, 0, 0.45), 0 0 25px rgba(201, 168, 76, 0.08) !important;
    }
    html .language-card.card-visible .card-image img,
    html .language-card.card-visible .card-image-stacked img {
...
- 头部标题缩小（0.85rem）+ 标题和年代触发器一行排列
- 桌面端 `.decade-nav` 隐藏，改为下拉菜单式 `.mobile-nav-wrap`（金色圆点 + 当前年代）
- 所有卡片自动高亮（`.card-visible`）— 不需点击，跟随滚动自动变化
- 语言切换按钮改为右下角浮动圆形按钮（EN/中）
...
<div class="page-footer">
  <small>
    <span class="footer-zh">© 2026 代码考古博物馆 · 归档人 <a href="https://github.com/cheneydc/codingMuseum">cheneydc</a> · 零人工手搓</span>
    <span class="footer-en">© 2026 Code Archaeology Museum · Archived by <a href="https://github.com/cheneydc/codingMuseum">cheneydc</a> · Zero human code</span>
  </small>
</div>
```
